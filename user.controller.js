...
export const updateMyProfile = async (req, res) => {
  ...
  try {
    // Get current headline to check if it changed
    const currentProfile = await pool.query(
      'SELECT headline FROM users WHERE user_id = $1',
      [req.user.id]
    );

    const oldHeadline = currentProfile.rows[0]?.headline;
    ...
    await pool.query(
      ...
      WHERE user_id = $7
      `,
      [displayName, bio, normalizedHeadline, linkedin, github, photoURL, req.user.id],
    );

    // If headline changed and user is student, create group if it doesn't exist
    if (normalizedHeadline && normalizedHeadline !== oldHeadline && req.user.role === 'student') {
      await pool.query(
        `
        ...
        `,
        [normalizedHeadline.toUpperCase(), req.user.id]
      );
    }
