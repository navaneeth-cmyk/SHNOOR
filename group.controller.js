    ...
    let query = `
      SELECT g.group_id, g.group_name, g.start_date, g.end_date, g.created_by, g.created_at,
             CASE 
               ...
               -- Timestamp groups: date-based cohorts (no college, active students only)
               ...
               -- Automatic college groups from groups table (active students only)
               ELSE (
                 SELECT COUNT(*)::int
                 FROM users u
...
export const getGroup = async (req, res) => {
  ...
  try {
    ...
    if (groupResult.rows.length > 0) {
      ...
      if (group.created_by) {
        // Manual group (random/college-like manual groups) - only active students
        ...
        // Timestamp group - only active students
        const userCountResult = await pool.query(
          `SELECT COUNT(*)::int AS user_count
           FROM users u
        ...
        // College manual group - only active students
        const userCountResult = await pool.query(
          `SELECT COUNT(*)::int AS user_count 
           FROM group_users gu
           JOIN users u ON gu.user_id = u.user_id
        ...
        // College automatic group (from groups table) - only active students
        const userCountResult = await pool.query(
          `SELECT COUNT(*)::int AS user_count
           FROM users u
           WHERE UPPER(u."headline/college_name") = UPPER($1) 
             AND u.role = 'student'
             AND u.status = 'active'`,
          [group.group_name]
        );
    ...
    try {
      ...
      if (collegeResult.rows.length > 0) {
        ...
        // Add user_count - only active students
        const userCountResult = await pool.query(
          `SELECT COUNT(*)::int AS user_count
           FROM users u
           WHERE u."headline/college_name" = $1 
             AND u.role = 'student'
             AND u.status = 'active'`,
          [group.group_name]
        );
...
export const getGroupUsers = async (req, res) => {
  ...
  try {
    ...
    if (groupCheck.rows.length > 0) {
      ...
      if (created_by) {
        // Manual group - only active students
        const result = await pool.query(
          `SELECT
             u.user_id,
             u.full_name,
      ...
      try {
        ...
        if (collegeCheck.rows.length > 0) {
          ...
          // Use exact match since college_groups.group_name is the actual college name - only active students
          const result = await pool.query(
            `SELECT
               u.user_id,
               u.full_name,
...
export const addUserToGroup = async (req, res) => {
  ...
  try {
    ...
    if (created_by || (groupStartDate && !groupEndDate)) {
      // Manual or college manual group - only allow active students
      const userCheck = await pool.query(
        `SELECT user_id FROM users WHERE user_id = $1 AND role = 'student' AND status = 'active'`,
        [userId]
      );
      if (userCheck.rows.length === 0) {
        return res
          .status(400)
          .json({ message: "Only active students can be added to groups" });
      }
      await pool.query(
