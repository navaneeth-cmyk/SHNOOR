...
/**
 * Normalize college name for flexible matching
 * Handles: case variations, extra spaces, special characters, abbreviations
 * Examples:
 *   "SRM University, AP" -> "SRM UNIVERSITY AP"
 *   "COMSATS Islamabad." -> "COMSATS ISLAMABAD"
      ...
      SELECT 
        ...
        CASE 
          ...
          ELSE (
            SELECT COUNT(*)::int
            FROM users u
            WHERE u."headline/college_name" IS NOT NULL
              AND REGEXP_REPLACE(UPPER(TRIM(u."headline/college_name")), '[,.\\-_() ]+', ' ', 'g') 
...
export const removeUserFromGroup = async (req, res) => {
  ...
  try {
    ...
    if (created_by) {
      ...
      // COLLEGE GROUP: Remove by clearing the college_name field for this user
      const userCheck = await pool.query(
        `SELECT "headline/college_name" FROM users WHERE user_id = $1 AND role = 'student' AND status = 'active'`,
        [userId]
      );
      ...
      // Only clear college_name if it matches the group name
      const userCollege = userCheck.rows[0]["headline/college_name"];
      if (userCollege) {
        ...
        if (normalizeForComparison(userCollege) === normalizeForComparison(group_name)) {
          await pool.query(
            `UPDATE users SET "headline/college_name" = NULL WHERE user_id = $1`,
            [userId]
          );
