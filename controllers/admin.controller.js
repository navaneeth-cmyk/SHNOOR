...
export const assignCourses = async (req, res) => {
  ...
  if ((!studentIds || studentIds.length === 0) && (!groupIds || groupIds.length === 0)) {
    return res.status(400).json({ message: "studentIds or groupIds are required" });
  }
  ...
  try {
    // Expand groups into studentIds via group_members view if groupIds provided
    let expandedStudentIds = [...studentIds];
    ...
    if (groupIds && groupIds.length > 0) {
      ...
      const groupStudentIds = groupRes.rows.map((r) => r.user_id);
      expandedStudentIds = Array.from(new Set([...expandedStudentIds, ...groupStudentIds]));
    }

    if (expandedStudentIds.length === 0) {
      return res.status(400).json({ message: "No students found for provided groups" });
    }
    ...
    for (const studentId of expandedStudentIds) {
    ...
    const query = `INSERT INTO course_assignments (student_id, course_id) VALUES ${placeholders.join(",")} ON CONFLICT DO NOTHING`;
